...
if __name__ == "__main__":
    ...
    JSON_INDENT = 4
    PRESETS_LIMIT = 5
    RESTRICTION_MESSAGE = "Cannot have more than {} presets. Remove presets with -rd/--regex-delete [name]".format(PRESETS_LIMIT)
    ...
    def restriction(msg=RESTRICTION_MESSAGE):
        sys.exit(msg)
    ...
    custom_regex.add_argument('-pr', '--preset-regex', help="finds regex by name from presets to replace it. Has less \
        priority than -r/--regex")
    ...
    config_args.add_argument('-ra', '--regex-add', nargs=2, metavar=('NAME', 'REGEX'), help="adds regex by name in regex presets.\
         Multiworded name needs to be quoted")
    config_args.add_argument('-rd', '--regex-delete', nargs='+', metavar='NAME', help="deletes one/multiple regex by it's \
        name from regex presets. Multiworded name needs to be quoted. Remove default with 'default'")
    ...
    if args.path and args.replacewith:
        ...
        if args.regex:
            ...
            try:
                ...
                sys.exit("No preset exists by that name. -rl/--regex-list gives a list of all presets")
